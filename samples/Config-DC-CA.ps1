...
#    Import-DscResource -ModuleName xActiveDirectory,xDisk, xNetworking, cDisk, PSDesiredStateConfiguration, xAdcsDeployment
    Import-DscResource -ModuleName xActiveDirectory,xDisk, xNetworking, PSDesiredStateConfiguration, xAdcsDeployment, cDisk
#    Import-DscResource -ModuleName cDisk

#    [System.Management.Automation.PSCredential ]$DomainCreds = New-Object System.Management.Automation.PSCredential ("$DomainName\$Admincreds.UserName", $Admincreds.Password)
	$DomainCreds = $AdminCreds
    ...
    Node localhost
    {
		...
		xADUser AccountToJoinDomain
		{
            Ensure = "Present"
            DomainName = $DomainName
            DomainAdministratorCredential = $DomainCreds
			UserName = $serviceAccountCreds.UserName
